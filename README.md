# Voice Agent Batch Testing Platform
...
Voice agents are hard to regression-test manually. Teams need automated persona-driven conversations that exercise the agent and score the results at scale.
...
1. Phase 1: Persona scripting format and simulated caller
2. Phase 2: Recording/transcription pipeline
3. Phase 3: LLM-as-judge scoring of conversations
4. Phase 4: Dashboard for pass/fail and latency trends

## Task Tracking

Work is broken into phase-tagged user stories tracked as GitHub Issues, not in this file. To see what's open:

    gh issue list --repo faheemkhaskheli9/voice-agent-evaluation --state open --label type:user-story
...
```bash
docker build -t voice-agent-evaluation .
docker run -p 8000:8000 voice-agent-evaluation
```
